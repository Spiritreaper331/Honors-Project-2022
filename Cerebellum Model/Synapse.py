...
class Synapse:
    def __init__(self, source, target, section, weight=1):
        ...
        if source == 'PF_NMDA':
            ...
            if isinstance(target, Golgi.Golgi):
                # Make a mossy fiber synapse onto a Golgi cell or Granule cell
                # Use deterministic synapses
                self.whatami = "syn_MF2GoC_det"
                self.postsyns['AMPA'] = [set_attrs(h.Golgi_MF_syn(0.5, sec=section), tau_facil=5, tau_rec=8, tau_1=1, gmax=1200, U=0.43)]
            ...
            if isinstance(target, Golgi.Golgi):
                # Make a mossy fiber NMDAB onto a Golgi cell or Granule cell
                # Use deterministic synapses*
        ...
        elif source == 'GoC_GABA': 
            if isinstance(target, GrC.GrC):
                # Make an inhibitory synapse onto a Granule cell
                # Use deterministic synapses*
                self.whatami = "syn_GoC2GrC_det"
                self.postsyns['GABA'] = [set_attrs(h.GRANULE_GABA_det_vi(0.5, sec=section), tau_facil=4, tau_rec=15, tau_1=3.61, gmax=2600, U=0.5)] 
